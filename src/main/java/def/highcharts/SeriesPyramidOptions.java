...
/**
 * (Highcharts) A `pyramid` series. If the type option is not specified, it is inherited from chart.type.
 *
 ...
 *
 * You have to extend the `SeriesPyramidOptions` via an interface to allow custom properties: ``` declare interface SeriesPyramidOptions { customProperty:
 * string; }
 *
 */
@jsweet.lang.Interface
@jsweet.lang.Extends({ SeriesOptions.class })
public abstract class SeriesPyramidOptions extends PlotPyramidOptions {
	/**
	 * (Highcharts) An array of data points for the series. For the `pyramid` series type, points can be given in the following ways:
	 *
	 ...
	 * sources.
	 */
	public def.highcharts.StringTypes.pyramid type;
	/**
	 * Not available
	 */
	@jsweet.lang.Optional
