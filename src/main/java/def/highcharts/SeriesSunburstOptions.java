 ...
 *
 * You have to extend the `SeriesSunburstOptions` via an interface to allow custom properties: ``` declare interface SeriesSunburstOptions { customProperty:
 * string; }
 *
 */
@jsweet.lang.Interface
@jsweet.lang.Extends({ SeriesOptions.class })
public abstract class SeriesSunburstOptions extends PlotSunburstOptions {
	/**
	 * (Highcharts) An array of data points for the series. For the `treemap` series type, points can be given in the following ways:
	 *
	 * 1. An array of numerical values. In this case, the numerical values will be interpreted as `value` options. Example: (see online documentation for
	 * example)
	 *
	 ...
	 * sources.
	 */
	public def.highcharts.StringTypes.sunburst type;
	/**
