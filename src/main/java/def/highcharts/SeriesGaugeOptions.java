...
/**
 * (Highcharts) A `gauge` series. If the type option is not specified, it is inherited from chart.type.
 *
 ...
 *
 * You have to extend the `SeriesGaugeOptions` via an interface to allow custom properties: ``` declare interface SeriesGaugeOptions { customProperty: string; }
 *
 */
@jsweet.lang.Interface
@jsweet.lang.Extends({ SeriesOptions.class })
public abstract class SeriesGaugeOptions extends PlotGaugeOptions {
	/**
	 * (Highcharts) An array of data points for the series. For the `gauge` series type, points can be given in the following ways:
	 *
	 ...
	 * sources.
	 */
	public def.highcharts.StringTypes.gauge type;
	/**
