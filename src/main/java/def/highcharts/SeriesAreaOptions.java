...
/**
 * (Highcharts, Highstock) A `area` series. If the type option is not specified, it is inherited from chart.type.
 *
 ...
 *
 * You have to extend the `SeriesAreaOptions` via an interface to allow custom properties: ``` declare interface SeriesAreaOptions { customProperty: string; }
 *
 */
@jsweet.lang.Interface
@jsweet.lang.Extends({ SeriesOptions.class })
public abstract class SeriesAreaOptions extends PlotAreaOptions {
	/**
	 * (Highcharts, Highstock) An array of data points for the series. For the `area` series type, points can be given in the following ways:
	 *
	 * 1. An array of numerical values. In this case, the numerical values will be interpreted as `y` options. The `x` values will be automatically calculated,
	 * either starting at 0 and incremented by 1, or from `pointStart` * and `pointInterval` given in the series options. If the axis has categories, these will
	 * be used. Example: (see online documentation for example)
	 *
	 * 2. An array of arrays with 2 values. In this case, the values correspond to `x,y`. If the first value is a string, it is applied as the name of the
	 * point, and the `x` value is inferred. (see online documentation for example)
	 *
	 * 3. An array of objects with named values. The following snippet shows only a few settings, see the complete options set below. If the total number of
	 ...
	 * sources.
	 */
	public def.highcharts.StringTypes.area type;
	/**
	 * Not available
	 */
	@jsweet.lang.Optional
	public Object useOhlcData;
	/**
	...
	public String name;
	/**
	 * (Highcharts, Highstock) This option allows grouping series in a stacked chart. The stack option can be a string or anything else, as long as the grouped
	 * series' stack options match each other after conversion into a string.
	 */
	@jsweet.lang.Optional
